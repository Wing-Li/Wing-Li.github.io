...
var passportLoginhtml = '<div class="passport-popup" id="login-popup">';
   ...
   passportLoginhtml += '<a class="phonedtm" href="#">手机动态码登录<i></i></a>',
	...
	passportLoginhtml +='<div class="getfreevideo">注册即赠送3天VIP，免费观看全部课程</div>';
	passportLoginhtml += '</div>';
...
//第三方登录后绑定
var passportReghtml = '<div class="passport-popup" id="registe-bindPhone">';
	passportReghtml += '<div class="layout-inner">';
	passportReghtml += '<div class="hd"><h2>免费获取VIP</h2><i class="close"></i></div>';
	passportReghtml += '<div class="bd">';
	passportReghtml += '<p class="instructions">你已<span id="dsfbox">QQ</span>登录成功，绑定手机提升账户安全，同时<br/>获赠<span class="passport-warn-color">3天</span>VIP会员资格，免费观看全部课程。</p>'
	passportReghtml += '<div class="passport-sign">';
	passportReghtml += '<div class="main-form">';
	passportReghtml += '<form class="passport-form passport-form-sign" id="register-form-popup" action="' + passportUrl + '/verify/phone_verify">';
	passportReghtml += '<div class="form-item">';
	passportReghtml += '<div class="form-cont">';
	passportReghtml += '<input type="text" name="phone" class="passport-txt xl w-full" tabindex="1" placeholder="请输入手机号" /></div>';
	passportReghtml +='</div>';
//	passportReghtml += '<div class="form-item">';
//	passportReghtml +='<div class="form-cont">';
//	passportReghtml +='<input type="password" name="password" class="passport-txt xl w-full" tabindex="2" placeholder="输入6~32位密码"/>';
//	passportReghtml +='<ul class="passport-safely" id="safely"><li class="danger">弱</li><li class="general">中</li><li class="safe">高</li></ul>';
//	passportReghtml +='</div></div>';
	passportReghtml +='<div class="form-item form-imgcode">';
	passportReghtml +='<div class="form-cont">';
	passportReghtml +='<div class="layout-inner2"><input type="text" name="verify" class="passport-txt xl w-lg" tabindex="3" placeholder="验证码"></div>';
	passportReghtml +='<div class="imgcode"><img src="" alt="" class="verifyCode" jktag="0001|0.1|91034" /><i class="passport-icon2 icon-refresh refreshCode" jktag="0001|0.1|91034"></i></div>';
	passportReghtml +='</div></div>';
	passportReghtml +='<div class="form-item form-mcode mb-25">';
	passportReghtml +='<div class="form-cont">';
	passportReghtml +='<input type="text" name="verify_code" class="passport-txt xl w-full" tabindex="4" placeholder="动态码"/>';
	passportReghtml +='<div class="btn-getcode"><button type="button" class="passport-btn js-getcode" jktag="0001|0.1|91035">获取动态码</button></div>';
	passportReghtml +='<div class="passport-sms getVoice" style="display: none;">未收到短信？使用<a href="javascript:;" class="js-getvoice" jktag="0001|0.1|91044">语音动态码</a></div>';
	passportReghtml +='<div class="passport-sms reVoice" style="display: none;"><span class="js-revoice"></span>，请注意接听来电</div>';
	passportReghtml +='</div></div>';
	passportReghtml +='<div class="form-item form-treaty">';
	passportReghtml +='<div class="form-cont"><input type="checkbox" name="treaty" checked="checked" value="treaty" />';
	passportReghtml +='<a href="' + passportWwwUrl + '/help/service.html" target="_blank" class="treaty">极客学院用户协议</a></div>';
	passportReghtml +='</div>';
	passportReghtml +='<div class="form-item">';
	passportReghtml +='<div class="form-cont"><input type="button" id="register-pop-submit" class="passport-btn passport-btn-def xl w-full" tabindex="5" value="确定" jktag="0001|0.1|91037" /></div>';
	passportReghtml +='</div></form>';
	passportReghtml +='</div>';
	passportReghtml +='<div class="passport-ad" id="reg-ad01"></div></div></div></div></div></div>';
...
var SIGN = {
    ...
    otherlogin:function(registerPop2){
		...
		if(uid != undefined && ohterlogin != undefined ){
			$.ajax({
				type:"get",
				url:"http://ws."+passportBaseUrl+"/user/current",
				async:true,
				dataType:'jsonp',
				success:function(data){
					// console.log(data);
					if(data.code == 200){
						var phone_status = data.data.phone_status;
						if(phone_status==0 && !flag){
							// alert('pop');
							registerPop2();
						}
					}
    ...
    login: function(form, button) {
        ...
        $uname.bind('blur', function() {
            ...
            if(window.hasSa) sa.track('login_popup_visit',{InputBox:'输入账号'});
        });
        $pwd.bind('blur', function() {
            var $this = $(this);
            var name = $this.attr('name');
            ...
            if(window.hasSa) sa.track('login_popup_visit',{InputBox:'输入密码'});
        });
        $verify.bind('blur', function() {
            var $this = $(this);
            var name = $this.attr('name');
...
//              }));
        ...
        $submit.bind('click', function() {
            ...
            if(window.hasSa) sa.track('login_popup_visit',{click:'点击登录按钮'});
...
//	                }));
    ...
    fastlogin: function(form, button) {
        ...
        $phone.bind('blur', function() {
        	//神策
        	if(window.hasSa) sa.track('ThirdParty_popup_visit',{InputBox:'输入手机号'});
            var $this = $(this);
            var name = $this.attr('name');
            var val = $.trim($this.val());
            var check = self.required({
        ...
        $verify.bind('blur', function() {
        	//神策
        	if(window.hasSa) sa.track('ThirdParty_popup_visit',{InputBox:'输入验证码'});
            var $this = $(this);
            var name = $this.attr('name');
            var val = $.trim($this.val());
            var check = self.required({
        ...
        $mcode.bind('blur', function() {
        	//神策
        	if(window.hasSa) sa.track('ThirdParty_popup_visit',{InputBox:'点击获取动态码'});
            var $this = $(this);
            var name = $this.attr('name');
            var val = $.trim($this.val());
            var check = self.required({
                    ...
                    success: function() {
                        ...
                        self.submit({
                            ...
                            success: function(data) {
                                ...
                                if (typeof stat != 'undefined') stat.efunc({po:91066, posId: 91095});
                                //神策
        							if(window.hasSa) sa.track('ThirdParty_popup_PhoneSuccess');
                            }
                        });
                    }
                }));
    ...
    signPop: function() {
            ...
            $(this).passportPopup({
                ...
                fn: function() {
                	...
                	$('#register-popup .close').on("click",function(){
                     if(window.hasSa) sa.track('ThirdParty_popup_leave');
                 })
                    // 埋点
        ...
        //第三方登录绑定
        var registerPop2 = function(fn) {
            // 调用注册弹层请使用.diaRegBtn, 不要使用其他 class 或 id
            // alert('registerPop2');
            $(this).passportPopup({
                popupId: 'registe-bindPhone',
                ...
                default:
                	...
                	// 神策埋点
                if(window.hasSa) sa.track('reg_popup_visit');
                $('#registe-bindPhone .close').on("click",function(){
                	PASSPORT.setCookie('closebindpop','bindphone',712);
                    if(window.hasSa) sa.track('ThirdParty_popup_leave');
                })
                    // 埋点
                    PASSPORT.efunc({posId:910005});
                    ...
                    self.register('#register-form-popup', '#register-pop-submit', true);
                    ...
                    // 获取动态码
                    $('#registe-bindPhone #register-form-popup .js-getcode').bind('click', function() {
